...
def deal_of_cards():    #Ta funkcja zadziała po naciśnięciu przycisku:Zagraj i rozda po dwie karty graczowi i krupierowi(jedna karta krupiera będzie zakryta).
    ...
    global button2
    global button3
    button3=Button(window, text = "Rozdwoić karty", bg="black", fg="white", command=split)
    button3.place(x=0, y=140)
...
## RUCH SPLIT I ROZGRYWKA W ZAGNIEŻDŻONYM OKNIE
def split():
    drugie_okno = tk.Tk()
    drugie_okno.title("Split")
    drugie_okno.geometry("800x500")
    drugie_okno.configure(background="green")
    opis_drugiego_pl =Label(drugie_okno, text = "Split!", fg = "black")
    gracz=[]
    krupier=[]
    ...
    button1 = Button(drugie_okno, text = "Dobierz kartę", bg="black", fg="white") # command=picking_card_player1_w_split)
    button1.place(x=0,y=50)
    button2 = Button(drugie_okno, text = "Nie dobieraj", bg="black", fg="white") # command=picking_card_croupier1_w_split)
    button2.place(x=0,y=95)

    deal_of_cards()
    picking_card_player1()
    picking_card_player2()
    picking_card_player3()
    picking_card_player4()
...
deck = os.listdir()
deck.remove("wersja_split.py")
deck.remove("CHYBA_OSTATECZNY.py")
deck.remove("purple_back.png")
# deck.remove("gjuu.py")
def los(deck):
    random_card=random.choice(deck)
